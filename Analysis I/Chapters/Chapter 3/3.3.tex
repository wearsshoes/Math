...
\begin{enumerate}
    ...
    \begin{proof}
        ...
        Thus it has the same domain and codomain as $g$.
        ...
        Suppose there exists another function $h: X \cup Y \to Z$ and $h': X \cup Y \to Z$ such that $h' \circ \iota_{X \to X \cup Y} = f$ and $h' \circ \iota_{Y \to X \cup Y} = g$ as well.
        We know that the domains and codomains of $h$ and $h'$ are the same.

        Since $h' \circ \iota_{X \to X \cup Y} = f$, and $\iota_{X \to X \cup Y}$ is an inclusion map, $h'=f$ for all $x \in X$. Likewise since $h' \circ \iota_{Y \to X \cup Y} = g$, and $\iota_{Y \to X \cup Y}$ is an inclusion map, $h'=g$ for all $y \in Y$. This means $h'$ has the same value as $h$ for $a \in X \cup Y$, so they are identical and $h$ is unique.
    \end{proof}
